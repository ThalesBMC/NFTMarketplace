...
export const CardProfile = ({ data,size }) => {
   ...
   const { users, getUsers, userInfo, walletId, removeFavorite, addFavorite,favoritedList } = useContext(LoginContext);
  ...
  return (
    <>
        ...
        <img
          className="w-full"
          src={data.image}
          style={{ height: size==="userFavorite"? "300px" :'450px',width: size==="userFavorite"? "300px" :'' }}
          alt="Sunset in the mountains"
        />
        <div className="px-6 py-4">
          <div className=" text-white font-bold text-xl mb-2"> {data.name}</div>
          <p className="text-white text-base overflow-ellipsis overflow-hidden">{data.description}</p>
        </div>
